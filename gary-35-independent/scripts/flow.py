...
def main():
 p=argparse.ArgumentParser();p.add_argument('action',choices=['prepare','expand']);p.add_argument('--source',type=Path);p.add_argument('--run-dir',type=Path,required=True);p.add_argument('--review',type=Path);p.add_argument('--proposal',type=Path);p.add_argument('--check-only',action='store_true');a=p.parse_args();r=a.run_dir.resolve()
 if a.action=='prepare':
  if not a.source:p.error('--source required')
  if not a.review or not a.proposal:p.error('--review and --proposal required for semantic dedup')
  dedup.verify(a.source,a.review)
  review_screen=json.loads(Path(json.loads(a.review.read_text())['screen']).read_text())
  assert review_screen['proposal_sha256']==sha(a.proposal.read_bytes()),'proposal changed'
  ...
  (r/'dedup_review.json').write_bytes(a.review.read_bytes());(r/'route.json').write_bytes(a.proposal.read_bytes())
  dedup.register(r)
  (r/'history_after_reserve.sha256').write_text(sha((dedup.HOME/'index.json').read_bytes()))
  print(json.dumps({'prepared':str(r),'source_cjk':n},ensure_ascii=False));return
 assert (r/'history_after_reserve.sha256').read_text()==sha((dedup.HOME/'index.json').read_bytes()),'history changed after reservation; repeat dedup review'
