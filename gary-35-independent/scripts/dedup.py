...
import argparse,json,hashlib,re,subprocess,sys,datetime
HOME=Path('/Users/kin/Documents/Codex/gary-shared-history')
...
def screen(source,proposal,output):
 rs=json.loads((HOME/'index.json').read_text());s=source.read_text();q=json.loads(proposal.read_text());wanted=json.dumps(q,ensure_ascii=False);rank=[];exact=[]
 for x in rs:
...
def verify(source,review):
 ...
 assert r.get('compared_ids') and r.get('point_comparison') and r.get('distinct_action_chain') and r.get('topic_fidelity'),'semantic evidence missing'
 return r

def register(run):
 source=run/'approved_source.txt';q=json.loads((run/'dedup_review.json').read_text());screen=json.loads(Path(q['screen']).read_text());proposal=json.loads((run/'route.json').read_text());row=dict(proposal);row.update({'id':digest(str(run.resolve()).encode())[:16],'status':'generated' if (run/'expanded_raw.txt').exists() else 'reserved','run':str(run.resolve()),'search_text':json.dumps(proposal,ensure_ascii=False),'artifacts_copied':[{'original':str(p),'text':p.read_text(),'sha256':digest(p.read_bytes())} for p in [source,run/'expanded_raw.txt'] if p.exists()]});dump(HOME/'new'/(row['id']+'.json'),row)
 rs=json.loads((HOME/'index.json').read_text());rs=[x for x in rs if x['id']!=row['id']]+[row];dump(HOME/'index.json',rs)

if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('action',choices=['refresh','screen','verify','register']);p.add_argument('--source',type=Path);p.add_argument('--proposal',type=Path);p.add_argument('--output',type=Path);p.add_argument('--review',type=Path);p.add_argument('--run-dir',type=Path);a=p.parse_args()
 if a.action=='refresh':refresh()
 elif a.action=='screen':screen(a.source,a.proposal,a.output)
 elif a.action=='verify':verify(a.source,a.review);print('semantic record verified')
 else:register(a.run_dir)
