...
def main() -> None:
    ...
    pack = {
        ...
        "audit_notes": ["匹配分为本地可审计文本相似度，不代表知识已被人工确认适用", "外部精炼知识卡保留来源编号、页码和连续OCR引文，实际采用后必须回写card_id", "匿名复合案例是叙事适配，不等于Gary真实学员证据", "跨来源模块保留证据等级，单一来源不升级为已验证规律", "历史发布证据来自扩写前原稿与后台数据连接；发布正文和实际开头未核验时不得做知识点或开头归因", "开头来自高赞样本，但点赞不被归因于单一开头", "本文件不是成稿，也没有调用扩写模型"],
    }
