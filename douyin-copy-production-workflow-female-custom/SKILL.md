...
## Female Edition Defaults

This skill is a copied female-facing edition of `douyin-copy-production-workflow-custom`. Do not modify or route through the male edition unless the user explicitly asks.

Apply these overrides throughout expansion, openings, title packaging, case selection, and local 2.7 fusion:

- Default audience: women / female viewers / 姐妹 / 女性学员.
- Default relationship lens: understand men, screen men, identify male investment, protect emotional sovereignty, rebuild female frame, and avoid low-position attachment.
- Default case protagonist: a female student/client unless the source clearly requires a male protagonist.
- Default analysis objects: male psychology, male behavior signals, male commitment/investment, female boundaries, female relationship position, and emotional self-command.
- Keep the user's configured brand/slogan unless the user gives a new female-brand name. If no new brand is specified, keep `探花Gary` in fixed endings.
- Preserve sharp Gary-style pressure and human-nature logic, but convert conquest/追女生 language into women-facing screening, frame, boundary, and emotional-sovereignty language.
- If a copied reference mentions `male relationship copy`, `brothers`, `兄弟`, `男人追女生`, or `女性被征服`, reinterpret it through this female-audience lens instead of following the male wording literally.
...
- Female edition isolation rule: use `--prompt-file <prompt> --isolated --output-file <expanded>` by default for this female-facing workflow. The current Mac runner is stateless by default, and the flag keeps the workflow explicit and separate from the male edition.
...
Build each UTF-8 prompt file with the `Female Edition Prompt Override` block from `references/gemini-expansion.md`, then the selected full Gemini instruction block from the same reference, followed immediately by the original source text wrapped as `【原文开始】...【原文结束】`. Do not shorten the selected 2.5/2.6 block; the female override is an added semantic layer. If the user asks for 单案例、一案到底、一个案例贯穿全文, use the `2.6 Single Case` direction.
