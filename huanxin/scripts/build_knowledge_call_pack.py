#!/usr/bin/env python3
"""Build an auditable route-neutral same-topic knowledge and history call pack."""
...
def main() -> None:
    ...
    lines += ["", "## 本次建议", "", f"- 主知识模块：{selected_cross[0]['module_id']} {selected_cross[0]['title']}" if selected_cross else "- 暂无足够匹配模块，需人工补充", "- 辅助知识只选能自然接在原母题后的2至4张卡，不做知识堆砌。", "- 完稿后必须回写实际使用的机制、操作、结构与状态。", ""]
