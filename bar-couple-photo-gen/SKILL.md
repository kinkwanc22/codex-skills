...
2. `酒店酒廊沙发抓拍照片提示词` - the fixed Chinese hotel-lounge sofa candid photo prompt below.

If the user chooses the hotel-lounge sofa photo prompt, use the following prompt text exactly as the core prompt. Keep it in Chinese; do not translate it into English. Use the current male/female references as identity references unless the user provides other references.
...
1. Before Gary-series generation, ask the user whether to use `真实抓拍照片提示词` or `酒店酒廊沙发抓拍照片提示词`, unless the choice is already explicit.
