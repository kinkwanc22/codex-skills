...
- `references/3.2-conversion-transplant.md`: read when the user asks for `3.2`, `3.2换芯`, `3.2换芯扩写`, or wants a finished Gary draft rebuilt with the old-2.5 attack level, conversion-oriented outer structure, and short cases used as evidence throughout the article.
...
If the user asks for `3.2`, `3.2换芯`, `3.2换芯扩写`, or `成交结构换芯`, use `references/3.2-conversion-transplant.md`. This is an independent finished-draft transplant mode, not a rename of 2.5成稿换芯 and not 3.1 source pre-transplant. Begin with the complete current 2.5 direct-draft prompt, then append the complete 3.2 task layer, an article-specific content card, limited-inheritance card, exclusions, and the approved finished-draft body. Preserve the body-derived mother topic, audience promise, one core mechanism, at most two short signature sentences, and the promised count; rebuild the remaining 70%-80% around the hidden path `强结论钩子 -> 精准场景 -> 错误动作 -> 暗黑机制 -> 现实后果 -> 盘点判断与方法 -> 个体变量缺口 -> 单一粉丝群CTA`. Short cases are normal evidence units: attach them after selected mechanisms or weave three to six compact cases through the article; do not turn them into one complete consultation-reversal story. If the user asks for one case through the whole article, switch to 2.6. Run 3.2 with the dedicated `--session 3.2-transplant` session and maintain separate route, case-layout, conversion-signature, similarity, and retry ledgers.
...
Do not use the old web expansion channel. Do not send `/new` unless the user explicitly asks. Routine 2.5 is isolated per source and does not write to a saved conversation; `2.5成稿换芯` and `3.2换芯扩写` are exceptions and use their own copied dedicated sessions. Keep 2.5-transplant, 2.8, 2.9, and 3.2-transplant in separate dedicated sessions, and do not run these directions through the default mixed `gemini_session.json`.
...
For `3.2换芯扩写`, apply the full validation gate in `references/3.2-conversion-transplant.md`. In addition to the transplant checks, reject drafts without concrete short-case evidence, drafts that expose the conversion structure as labels, drafts that become a case collection or a full consultation-reversal story, and drafts that introduce more than one conversion objective. Verify that the public content is substantial before the personal-variable gap, every case returns to a mechanism judgment, the CTA requests topic-specific evidence, and the last sentence is exact.
