...
description: Complete Douyin copy production workflow. Use for 抖音文案工作流, 处理这批文案, Gemini本地命令扩写, 3.1知识库换芯, 3.1H混合换芯, Codex先设计知识库辅助, 3.1中段CTA, 3.2换芯扩写, 3.3独立换芯扩写, 旧版3.1改为3.3, Codex独立设计新观点, 2.5成稿换芯, 三版开头, 长文稿Word导出, 3.0千川素材, 保头压中自然转化, 热门原稿植入CTA, 1500字内成稿, 直接TTS Word, and end-to-end male relationship copy production.
---
...
- `references/3.1H-codex-first-knowledge-supported.md`: read in full when the user asks for `3.1H`, `3.1H混合换芯`, `Codex先设计知识库辅助`, `知识库只补充弹药`, or explicitly approves the September 3 hybrid test logic. Codex first designs one article-level causal spine and same-level point hierarchy; the knowledge base then supports mechanisms, evidence, phrasing, and one or two sourced Gary student mini-cases without determining the public skeleton. Gemini expands the frozen source through the complete old 2.5 prompt plus the builder-owned `3.1H-natural-rhythm-lock.txt` with `--isolated`; never append a second hand-written naturalness layer.
...
If the user asks for `3.1H`, `3.1H混合换芯`, `Codex先设计＋知识库辅助`, `知识库只补充弹药`, or the fixed September 3 hybrid logic, follow `references/3.1H-codex-first-knowledge-supported.md`. Keep this mode separate from default 3.1: Codex may independently design the article-level thesis, causal spine, point hierarchy, and transitions; retrieved knowledge is supporting material rather than the source of public point headings. Use one or two compact, source-backed Gary student cases after the relevant theory. Build the Gemini request only with `scripts/build_31h_gemini_prompt.py`; it automatically combines the complete old 2.5 block, exact mother-topic lock, and fixed natural-rhythm lock. Run one fresh `--isolated` request, preserve the old-2.5 hard surface, and narrow any model-added case facts back to the frozen evidence boundary before Word export. Once the body passes the mandatory repair boundary, do not add a separate Codex semantic-polish stage; optional comments about rhythm, evidence, metaphors, or title style do not authorize local rewriting. Do not append another naturalness prompt and do not silently route a plain `3.1` request into 3.1H.
...
2. Extract `开头版本三：保留原文开头（来自源文档）` from the source document without rewriting it.
   - For 3.1 and 3.1H, also add `开头版本四：原文开头优化版（贴合正文）` according to the active mode reference. Keep version three verbatim and keep version four separate from the accepted body.
...
For 3.1 and 3.1H, do not run LibreOffice, `soffice`, PDF conversion, PNG rendering, or `render_docx.py`. Formal 3.1 saves the existing Gemini-retention manifest and runs `scripts/validate_31_final_package.py`. For 3.1H, save the raw/final hashes, exact `case_fact_repairs`, terminal ending replacement, heading/body checks, ZIP integrity, file size, and timestamp in its run package; highlight only the normalized fixed ending unless the user requests a clean copy.
