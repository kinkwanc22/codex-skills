...
For 3.1, route diversity must never be purchased by lowering the force band. Every replacement point must contain a memorable Gary judgment and show at least one concrete interest shift, relationship-position change, subconscious revaluation, selection consequence, emotional reward/punishment, or real cost. Reject a route that is merely accurate, professional, or novel but centers neutral management, risk control, healthy communication, collaboration, psychological nourishment, evidence classification, or counseling language. Those ideas may support a harder argument, but they cannot replace the source's sharper attack direction unless the source itself is explicitly about that neutral object. Record `force_band: old_2.5_hard`, `memorable_judgments`, and `interest_shifts` in the 3.1 ledger before freezing the rebuilt source.

For 3.1 technique/how-to manuscripts, changing only the explanation is not a transplant. Detect this class from the body when it promises concrete phrases, steps, scripts, invitation methods, chat replies, progression moves, timing methods, or other executable actions. Keep the body-derived mother topic, target result, relationship stage, and promised item count, but replace the source's actual techniques: the action, wording, sequence, trigger, timing, scene, example, and proof chain. A renamed old action, an old script under a new psychological term, or the same operation with a different example still counts as reuse and must be rebuilt. Each replacement technique must state when to use it, what to say or do, what observable feedback to watch for, and the next move or failure condition. Preserve an old technique only when the user explicitly requests it. Record `content_class: technique`, `source_techniques`, `replacement_techniques`, and `operation_overlap` in the 3.1 ledger before freezing.
