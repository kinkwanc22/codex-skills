...
If the user asks for `2.5成稿换芯`, `用扩写好的2.5改内容`, `保留母题替换内容`, or says to keep the method approved by the August 1 tests, do not use routine source expansion. Read `references/2.5-finished-draft-transplant.md`. Prompt assembly must begin with the complete July 5 original 2.5 instruction block; append the transplant/strong-attack task layer after it, then add the approved 2.5 finished draft as the style parent. Never use the new transplant layer by itself. Extract and blacklist the style parent's concrete old items, design a genuinely new content skeleton under the same mother topic, then run the fused prompt in one isolated request.
