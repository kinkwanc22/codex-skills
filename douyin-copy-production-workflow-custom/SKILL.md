...
description: Complete Douyin copy production workflow. Use for 抖音文案工作流, 处理这批文案, Gemini本地命令扩写, 3.1知识库换芯, 3.1均衡软植入, 2.5成稿换芯, 3.2换芯扩写, 三版开头, 长文稿Word导出, 3.0千川素材, 保头压中自然转化, 热门原稿植入CTA, 1500字内成稿, 直接TTS Word, and end-to-end male relationship copy production.
---
...
Do not add risk suggestions or `[[RISKNOTE:...]]` markers. For 3.1, yellow highlight is reserved for Codex-added conversion text and is required by default; source/Gemini body text must remain unhighlighted.
...
- `references/3.1-balanced-soft-fan-group-layer.md`: read after every CTA-free accepted 3.1 Gemini body and before Word export. Codex adds article-native balanced soft placements, one topic-native mid CTA, and the exact fixed ending in a single post-expansion pass, then records and yellow-highlights those insertions by default.
...
If the user asks for `3.1`, `3.1换芯`, `2.5预换芯`, `原稿预换芯`, or `原稿先换芯再2.5`, read and follow `references/3.1-knowledge-base-transplant.md` as the canonical default. Infer and preserve the mother topic/count from the body; retrieve traceable knowledge, exclude accepted routes, and bind every replacement point to exact IDs. Codex may select, merge compatible knowledge, adapt perspective, polish, order, and write transitions, but it may not invent substantive content. Save point-level provenance and freeze a CTA-free source with `codex_content_invention: false`. Send the complete standard 2.5 block plus the fixed CTA-free 3.1 override and frozen source through `--isolated`; Gemini remains free to expand content but not conversion. Do not send vault paths, knowledge IDs, provenance, exclusions, or another supplement. Preserve Gemini's aggressive old-2.5 language surface except for narrow mechanical cleanup. If the knowledge base cannot support the promised count, stop and report the missing slot; do not silently invent or switch modes.

For 3.1, Gemini expansion must contain zero CTA, including zero Doubao/AI-summary prompt, engagement prompt (`点赞/收藏/评论/转发/艾特`), fan-group invitation, and no Gary fixed ending. Use the complete old 2.5 block plus the fixed CTA-free 3.1 override, then reject/retry any raw expansion that contains conversion language instead of deleting it after generation. After the CTA-free body passes topic/count/force validation, apply `references/3.1-balanced-soft-fan-group-layer.md` before titles/openings and Word export. Codex performs the full conversion pass once: add article-native balanced soft placements, one topic-native mid-article CTA by default, and the exact fixed ending. This is a narrow post-generation layer, not permission to rewrite the argument. In the default 3.1 Word, highlight every Codex-added soft placement, CTA, and fixed ending in yellow; do not highlight source/Gemini text.
...
Before final delivery, verify each `.docx` contains the required headings and has no risk suggestions or `[[RISKNOTE:...]]` markers. For 3.1, also verify the yellow-highlight set exactly matches the Codex insertion manifest and contains no other text.

For 3.1, do not run LibreOffice, `soffice`, PDF conversion, PNG rendering, or `render_docx.py`. Save an insertion manifest containing the exact highlighted texts, then run `scripts/validate_31_final_package.py --insertion-manifest <manifest.json>` once after the Word is saved. The unified check combines the mother-topic lock, forbidden-term scan, exact-ending check, four-opening/Word-structure check, accepted-body equality check, DOCX package integrity, and exact yellow-highlight-set check. Do not repeat those checks separately after the report passes. Use `--allow-no-yellow` only when the user explicitly requests a clean unhighlighted 3.1 Word.
