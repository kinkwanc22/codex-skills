...
For 3.2 knowledge retrieval, the standard call pack must include page-anchored `external_atomic_knowledge` and `GCOMP-*` Gary anonymized composite case candidates. If either is adopted, keep its ID and source status in the internal ledger; natural Gary first-person narration is allowed, but an external composite may not be relabelled internally as a verified Gary student record.
