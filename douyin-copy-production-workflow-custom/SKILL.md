...
If the user asks for `2.5成稿换芯`, `用扩写好的2.5改内容`, `保留母题替换内容`, or says to keep the method approved by the August 1 tests, use the current canonical fused transplant mode in `references/2.5-finished-draft-transplant.md`; do not use routine source expansion or the retired layer-only transplant prompt. Prompt assembly must begin with the complete July 5 original 2.5 instruction block; append the transplant/strong-attack task layer after it, then add the article-specific new skeleton and the approved 2.5 finished draft as the style parent. Never use the new transplant layer by itself. Extract and blacklist the style parent's concrete old items, design a genuinely new content skeleton under the same mother topic, then run the fused prompt in one isolated request.
...
For `2.5成稿换芯`, also reject any draft that paraphrases the old list, reuses an old signature case, carries over old dialogue, or changes the mother topic. Run an awkward-expression pass for invented compounds such as `位置稳的男人` or `性缩力`; repair only obvious local wording defects without softening the argument. Verify the new promised count, mid-article and ending fan-group CTAs, and the exact fixed ending.
