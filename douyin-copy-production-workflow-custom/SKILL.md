...
- `references/3.1H-codex-first-knowledge-supported.md`: read in full when the user asks for `3.1H`, `3.1H混合换芯`, `Codex先设计＋知识库辅助`, `知识库只补充弹药`, or explicitly approves the September 3 hybrid test logic. Codex first designs one article-level causal spine and same-level point hierarchy; the knowledge base then supports mechanisms, evidence, phrasing, and one or two sourced Gary student mini-cases without determining the public skeleton. Gemini expands the frozen source through the complete old 2.5 prompt with `--isolated`.
