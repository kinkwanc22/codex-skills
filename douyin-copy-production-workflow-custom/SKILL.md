...
- `references/3.1-cta-layer.md`: read after every cleaned zero-CTA 3.1 Gemini body is accepted and before Word export. Codex restores the fixed 2.5 opening guidance immediately after the first substantive opening paragraph, adds exactly one separate topic-native mid CTA, and appends the exact fixed ending in a single post-expansion pass. The opening guidance must never be moved into the middle. Scattered soft placements are forbidden; only these three units are recorded and yellow-highlighted by default.
...
For 3.1, preserve the complete old 2.5 prompt even though it asks Gemini to generate Doubao/AI-summary guidance, engagement prompts, fan-group conversion, account language, and its old fixed ending. Save that raw output unchanged. Then run `scripts/clean_31_gemini_conversion.py` to remove only clearly separable conversion units, save the exact deletion manifest, and validate the cleaned Gemini body with `scripts/validate_31_gemini_no_cta.py`. Retry Gemini only when conversion cannot be removed without damaging substantive content, the public mother topic/count drifts, length fails, or the output is a refusal/stale-topic response. After the cleaned body passes topic/count/force validation, apply `references/3.1-cta-layer.md` before titles/openings and Word export. Codex performs the conversion pass once: restore the exact fixed 2.5 opening guidance immediately after the first substantive opening paragraph, add exactly one separate topic-native mid-article CTA, and append the exact fixed ending. Never move the fixed opening guidance into the middle or reuse it as the mid CTA. Do not add scattered soft placements, repeated self-references, or extra fan-group mentions. This is a narrow post-generation layer, not permission to rewrite the argument. In the default 3.1 Word, highlight only the fixed opening guidance, Codex-added mid CTA, and fixed ending in yellow; do not highlight source/Gemini text.
