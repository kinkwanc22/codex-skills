...
- `references/3.2-conversion-transplant.md`: read when the user asks for `3.2`, `3.2换芯`, or `3.2换芯扩写`. This mode copies the complete 3.1 pre-transplant architecture, adds short-case design to the rebuilt source, then runs the frozen source through the same complete 2.5 prompt with `--isolated`.
...
If the user asks for `3.2`, `3.2换芯`, `3.2换芯扩写`, or `带小案例的预换芯`, use `references/3.2-conversion-transplant.md`. The canonical 3.2 is now a copied-and-modified 3.1 pre-transplant workflow, not a finished-draft transplant prompt layer. Apply the complete 3.1 body-first fingerprint, route-diversity, terminology, conversion-handoff, freeze, expansion, and validation architecture, but make the rebuilt source carry a planned short-case system: normally three to six compact cases, contrasts, or dialogue slices distributed across the new points. Each case must show a topic-specific action or dialogue, observable feedback, and the mechanism consequence, without becoming a full consultation-reversal story. Record 3.2 in its own ledger. After freezing the rebuilt source, send only the complete standard 2.5 direct-draft block plus that source through `--isolated`, exactly like 3.1. Do not append a 3.2 task layer, style parent, limited-inheritance card, exclusions, or dedicated-session history to the Gemini request. If the user asks for one case through the whole article, switch to 2.6.
...
For `3.2换芯扩写`, apply the complete inherited 3.1 validation gate plus the case gate in `references/3.2-conversion-transplant.md`. Reject any run whose frozen rebuilt source was not created and recorded before expansion, whose Gemini prompt contains a second-layer 3.2 instruction, whose cases are generic or detach from the current point, or whose final 2.5 draft collapses the planned short cases back into one consultation-flip story. Verify the separate 3.2 route/case/conversion ledger and the exact ending.
