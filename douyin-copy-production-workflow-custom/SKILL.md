...
If the user asks for `2.5成稿换芯`, `用扩写好的2.5改内容`, `保留母题替换内容`, or says to keep the method approved by the August 1 tests, do not use routine source expansion. Read `references/2.5-finished-draft-transplant.md` and treat the supplied approved 2.5 finished draft as a style parent only: extract and blacklist its content genes, design a genuinely new content skeleton under the same mother topic, then send the style parent and new skeleton in one isolated request.
