...
description: Complete Douyin copy production workflow. Use for 抖音文案工作流, 处理这批文案, Gemini本地命令扩写, 3.1知识库换芯, 3.1均衡软植入, 3.2换芯扩写, 3.3独立换芯扩写, 旧版3.1改为3.3, Codex独立设计新观点, 2.5成稿换芯, 三版开头, 长文稿Word导出, 3.0千川素材, 保头压中自然转化, 热门原稿植入CTA, 1500字内成稿, 直接TTS Word, and end-to-end male relationship copy production.
---
...
- `references/3.1-balanced-soft-fan-group-layer.md`: read after every cleaned zero-CTA 3.1 Gemini body is accepted and before Word export. Codex adds article-native balanced soft placements, one topic-native mid CTA, and the exact fixed ending in a single post-expansion pass, then records and yellow-highlights those insertions by default.
...
If the user asks for `3.1`, `3.1换芯`, `2.5预换芯`, `原稿预换芯`, or `原稿先换芯再2.5`, read and follow `references/3.1-knowledge-base-transplant.md` as the canonical default. Infer and preserve the mother topic/count from the body; retrieve traceable knowledge, exclude accepted routes, and bind every replacement point to exact IDs. Codex may select, merge compatible knowledge, adapt perspective, polish, order, and write transitions, but it may not invent substantive content. Save point-level provenance and freeze a conversion-free source with `codex_content_invention: false`. Build the Gemini request with `scripts/build_31_gemini_prompt.py`: the complete standard 2.5 block remains intact, the fixed public-mother-topic exact-lock block is inserted immediately before the frozen source, and every source runs through `--isolated`. Do not send vault paths, knowledge IDs, provenance, exclusions, or another supplement. Preserve Gemini's aggressive old-2.5 language surface except for exact conversion cleanup and narrow mechanical repair. If the first retrieval set cannot support the promised count, broaden retrieval and replace weak slots with other traceable B/X/external-atomic modules before blocking; never fill a slot from general model knowledge or silently switch modes.

For 3.1, preserve the complete old 2.5 prompt even though it asks Gemini to generate Doubao/AI-summary guidance, engagement prompts, fan-group conversion, account language, and its old fixed ending. Save that raw output unchanged. Then run `scripts/clean_31_gemini_conversion.py` to remove only clearly separable conversion units, save the exact deletion manifest, and validate the cleaned Gemini body with `scripts/validate_31_gemini_no_cta.py`. Retry Gemini only when conversion cannot be removed without damaging substantive content, the public mother topic/count drifts, length fails, or the output is a refusal/stale-topic response. After the cleaned body passes topic/count/force validation, apply `references/3.1-balanced-soft-fan-group-layer.md` before titles/openings and Word export. Codex performs the full conversion pass once: add article-native balanced soft placements, one topic-native mid-article CTA by default, and the exact fixed ending. This is a narrow post-generation layer, not permission to rewrite the argument. In the default 3.1 Word, highlight every Codex-added soft placement, CTA, and fixed ending in yellow; do not highlight source/Gemini text.
