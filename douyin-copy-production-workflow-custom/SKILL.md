...
- `references/3.1-cta-layer.md`: read after every cleaned zero-CTA 3.1 Gemini body is accepted and before Word export. Codex adds exactly one topic-native mid CTA and the exact fixed ending in a single post-expansion pass. Scattered soft placements are forbidden; only the CTA and ending are recorded and yellow-highlighted by default.
