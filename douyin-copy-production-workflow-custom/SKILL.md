...
- `references/3.2-conversion-transplant.md`: read when the user asks for `3.2`, `3.2换芯`, or `3.2换芯扩写`. This mode copies the complete 3.1 pre-transplant architecture, adds short-case design and the approved hidden monetization sequence to the rebuilt source, replaces 3.1's dual-CTA placement with one ending fan-group CTA, then runs the frozen source through the same complete 2.5 prompt with `--isolated`.
...
If the user asks for `3.2`, `3.2换芯`, `3.2换芯扩写`, or `带小案例的预换芯`, use `references/3.2-conversion-transplant.md`. The canonical 3.2 is now a copied-and-modified 3.1 pre-transplant workflow, not a finished-draft transplant prompt layer. Apply the complete 3.1 body-first fingerprint, route-diversity, terminology, freeze, expansion, and validation architecture. Add a planned short-case system: normally three to six compact cases, contrasts, or dialogue slices distributed across the new points. Also apply the 3.2 conversion override: hide the sequence `强结论钩子 -> 精准场景 -> 错误动作 -> 暗黑机制 -> 现实后果 -> 盘点判断与方法 -> 个体变量缺口 -> 单一结尾粉丝群CTA`, fully deliver the public answer before conversion, and remove 3.1's inherited mid-article explicit CTA. Each case must show a topic-specific action or dialogue, observable feedback, and the mechanism consequence, without becoming a full consultation-reversal story. Record 3.2 in its own ledger. After freezing the rebuilt source, send only the complete standard 2.5 direct-draft block plus that source through `--isolated`, exactly like 3.1. Do not append a 3.2 task layer, style parent, limited-inheritance card, exclusions, conversion supplement, or dedicated-session history to the Gemini request. If the user asks for one case through the whole article, switch to 2.6.
...
For `3.2换芯扩写`, apply the complete inherited 3.1 validation gate plus the case and monetization gates in `references/3.2-conversion-transplant.md`. Reject any run whose frozen rebuilt source was not created and recorded before expansion, whose Gemini prompt contains a second-layer 3.2 instruction, whose cases are generic or detach from the current point, whose final 2.5 draft collapses the planned short cases back into one consultation-flip story, whose useful public answer is withheld, or whose body contains a mid-article explicit CTA or more than one conversion action. Verify the separate 3.2 route/case/conversion ledger, the single ending conversion unit, and the exact ending.
