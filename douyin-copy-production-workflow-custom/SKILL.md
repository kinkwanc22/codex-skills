...
- `references/transplant-mechanism-novelty-gate.md`: read for every Gary transplant mode. Apply its six-field causal-route comparison before freezing the rebuilt source, include `generated / 正文待确认` drafts in the recent window, and write the current route back immediately so the next article cannot reuse it. This is a Codex selection gate and must never be appended to or replace the complete 2.5 Gemini prompt.
...
- `references/3.5-codex-first-knowledge-supported.md`: read in full when the user asks for `3.5`, `3.5混合换芯`, `Codex先设计知识库辅助`, `知识库只补充弹药`, or explicitly approves the September 3 hybrid test logic. Codex first designs one article-level causal spine and same-level point hierarchy; the knowledge base then supports mechanisms, evidence, phrasing, and one or two sourced Gary student mini-cases without determining the public skeleton. Gemini expands the frozen source through the complete old 2.5 prompt plus the builder-owned `3.5-natural-rhythm-lock.txt` with `--isolated`; never append a second hand-written naturalness layer.
- `references/3.5-experience-anchor-library.md`: read during 3.5 frozen-source design when using Gary experience anchors. Approved wording is exact and optional; select and fill only a topic-matched anchor inside the frozen source, and never append the whole library or its IDs to the Gemini request.
...
For every Gary transplant mode, apply `references/transplant-mechanism-novelty-gate.md` before the rebuilt source is frozen. A different title, term set, example, point order, or knowledge-ID combination does not count as a new route when the dominant causal machinery repeats. Register each `generated / 正文待确认` route before selecting the next article; do not wait for user acceptance or publication.

If the user asks for `3.5`, `3.5混合换芯`, `Codex先设计＋知识库辅助`, `知识库只补充弹药`, or the fixed September 3 hybrid logic, follow `references/3.5-codex-first-knowledge-supported.md`. Keep this mode separate from default 3.1: Codex may independently design the article-level thesis, causal spine, point hierarchy, and transitions; retrieved knowledge is supporting material rather than the source of public point headings. Use one or two compact, source-backed Gary student cases after the relevant theory. When an experience anchor is useful, also read `references/3.5-experience-anchor-library.md`, select one approved expression during frozen-source design, and record its ID; do not send the whole library to Gemini. Build the Gemini request only with `scripts/build_35_gemini_prompt.py`; it automatically combines the complete old 2.5 block, exact mother-topic lock, and fixed natural-rhythm lock. Run one fresh `--isolated` request, preserve the old-2.5 hard surface, and narrow any model-added case facts back to the frozen evidence boundary before Word export. Once the body passes the mandatory repair boundary, do not add a separate Codex semantic-polish stage; optional comments about rhythm, evidence, metaphors, or title style do not authorize local rewriting. Do not append another naturalness prompt and do not silently route a plain `3.1` request into 3.5.

`3.1H` is the retired name of 3.5. Historical files, work IDs, and paths may retain `3.1H` for traceability. If the user uses the old name in a new request, execute the 3.5 workflow and use `3.5` in all new prompts, ledgers, filenames, and delivery labels.
...
For 3.1 experience judgments, default to one source-supported judgment inside every core point of the frozen source. Treat this as a proof rule, not a visible writing template. Before freezing, build the adaptive argument architecture required by `references/3.1-knowledge-base-transplant.md`: classify the promised count, create one escalating whole-article spine, group long lists into hidden chapters, and allocate unequal depth to pivotal versus supporting points. A two-to-five-point article must receive deep sustained argument rather than list compression; a ten-plus-point article must gain chapter-level continuity rather than fifteen isolated mini-essays. Rotate materially different evidence operations across the article, such as observing a small decision, auditing a display surface, checking reaction timing, comparing written screening standards, inspecting a message sequence, or testing continuous behavior. Most judgments should be embedded as observable criteria; explicit `我判断/我观察/我复盘` authority phrasing is limited and may not become the recurring point opener. When a long same-topic list genuinely requires reuse of one evidence lane, change the observable criterion, scene, and diagnostic purpose rather than repeating the same paragraph shell. Do not fabricate client volume, identities, screenshots, guaranteed outcomes, or success stories. Reject a source when multiple points repeat `我经常/我复盘过很多/群里有个兄弟` with only the topic noun changed, or when the result reads as disconnected checklist items without cumulative pressure. Record the architecture class, hidden chapter map, point role, proof function, observable evidence, and supporting knowledge IDs in provenance. Do not append a Gemini instruction about experience density/count/type, and do not inject experience paragraphs after generation; Gemini still receives only the complete old 2.5 prompt, fixed mother-topic lock, and frozen source.
...
For 3.1 and 3.5, do not run LibreOffice, `soffice`, PDF conversion, PNG rendering, or `render_docx.py`. Formal 3.1 saves the existing Gemini-retention manifest and runs `scripts/validate_31_final_package.py`. For 3.5, save the raw/final hashes, exact `case_fact_repairs`, terminal ending replacement, heading/body checks, ZIP integrity, file size, and timestamp in its run package; highlight only the normalized fixed ending unless the user requests a clean copy.
