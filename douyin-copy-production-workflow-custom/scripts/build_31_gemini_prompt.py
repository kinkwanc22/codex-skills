#!/usr/bin/env python3
"""Build a 3.1 Gemini prompt from the complete old 2.5 block and exact topic lock."""
...
原稿数字不构成本轮硬锁；以“本轮成稿数量承诺”为唯一数量合同。全文收束时再次逐字使用“{public_topic}”；如本轮有数量承诺，必须完整兑现且不得临时增减。"""
    ...
    metadata = {
        "mode": "3.1_mother_topic_locked_old_2.5",
        ...
        "cta_free_override_present": "3.1扩写阶段转换隔离" in prompt,
    }
    if metadata["cta_free_override_present"]:
        raise ValueError("retired CTA-free override leaked into prompt")
