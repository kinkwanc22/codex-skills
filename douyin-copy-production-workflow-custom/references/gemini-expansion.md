...
For `2.8 Safe Draft` and routine `2.9 Fusion Draft` work, follow their dedicated-session rules later in this file. The revised 3.2 follows 3.1 and uses a fresh isolated 2.5 request per source. Do not send `/new` unless the user explicitly asks.
...
- `3.5 Codex-First, Knowledge-Supported Hybrid -> Mother-Topic-Locked 2.5 Expansion` / `3.5混合换芯`: use only when the user explicitly asks for 3.5, Codex先设计＋知识库辅助, or the approved September 3 hybrid logic. Read `references/3.5-codex-first-knowledge-supported.md`, `references/3.5-old-2.5-structure-engine.md`, and `references/3.5-gary-voice-session.md` in full. Codex designs one coherent article-level spine, then validates a topic-native master memory spine, causal acts, governing judgments, rotated proof carriers, and identity shift through `old_25_engine_plan.json`; knowledge supports mechanisms, proof, language, and one or two sourced Gary student mini-cases. Build with `scripts/build_35_gemini_prompt.py --engine-plan <old_25_engine_plan.json>`, stage the dedicated Gary voice session into the work package, run the candidate with `--session-file`, and narrow unsupported model-added case facts back to the frozen evidence boundary. Do not use `--isolated`, `--new-session`, or the mixed default session for formal 3.5.
- `3.6 Safe Transplant -> 2.8 Safe Draft Expansion` / `3.6安全换新扩写`: use when the user asks for 3.6, 安全换新扩写, or 先按3.5换新再用2.8扩写. Read `references/3.6-safe-transplant.md` in full. Reuse the complete 3.5 pre-expansion route design, same-topic lineage exclusion, three-candidate selection, adjustable count, knowledge support, old-2.5 structure-engine plan, and lean frozen source; then build only with `scripts/build_36_gemini_prompt.py`, which combines the complete 2.8 block with the fixed 3.6 topic/count/content-line lock. Run routine production with `--session 3.6`; do not use the 3.5 voice candidate, ordinary 2.8 session, or mixed default session.
- `3.1 Knowledge-Base Transplant -> Mother-Topic-Locked 2.5 Expansion + Gemini Full Retention` / `2.5原稿预换芯`: the canonical default for 3.1. Read `references/3.1-knowledge-base-transplant.md` in full. Codex builds the frozen source only from traceable knowledge. Gemini remains unrestricted by that knowledge closure and receives the complete standard 2.5 prompt, the fixed mother-topic exact-lock block, and the frozen source through `--isolated`. Save the raw response and preserve all returned content. Add no Codex CTA; normalize only the terminal ending to `我是探花Gary，我们粉丝群里见，感谢观看。`
...
紧接着必须植入引导语：“另外说一下，想节省时间的话，可以点个收藏，在评论区艾特豆包，让豆包给你总结出精髓或者思维导图后再回来观看，如果现实中有任何推进问题的话，也可以随时进入我的粉丝群提问。好，我们直接进入正题。”
...
The canonical default changed on 2026-08-29 and its post-expansion boundary changed again on 2026-09-01. Read and follow `references/3.1-knowledge-base-transplant.md` in full before acting. Codex uses a closed knowledge set for substantive source design; Gemini uses the complete standard 2.5 prompt plus the fixed mother-topic exact-lock block, without a knowledge-closure or CTA-free supplement. The workflow and source rules below are retained as the explicit legacy Codex-designed fallback and apply only when the user asks for `旧版3.1` or explicitly asks Codex to design new viewpoints. The canonical knowledge-base mode inherits only the non-conflicting shared rules: body-derived public topic, source count recorded as reference, a separately selected final count, history exclusion, hard force band, no backstage leakage, `--isolated`, full Gemini-content retention, narrow non-deleting mechanical repair, and terminal-ending normalization.
...
3. Create a topic fingerprint before inventing replacement points. Record the exact body-derived mother topic, audience, relationship stage, promised result, central object, usable real-world scenes, and the source mechanism family that must not be lightly renamed. Do not copy filename language into the fingerprint unless the same idea is independently present in the body.
4. Inspect the last 20 accepted 3.1 entries across all manuscripts, not only previous runs of the same title. From the Gemini workspace, run `python3 /Users/kin/.codex/skills/douyin-copy-production-workflow-custom/scripts/inspect_31_recent_routes.py --ledger work/2.5_pretransplant_ledger.json`, then choose a topic-native mechanism family that is not saturated in the recent ledger. Anti-repetition is not permission to lower the force band: never select a softer route merely because a hard-edged family was used recently.
5. Build a short pre-transplanted source manuscript before calling Gemini for the long 2.5 draft. The rebuilt source should keep the same body-derived topic and any count explicitly promised by the body, but replace the internal points, proof scenes, examples, mechanism order, and concrete signals. Every point must contain a scene anchor specific to this topic.
...
9. Keep conversion logic out of the rebuilt manuscript. Gemini may generate the conversion language already required by the complete old 2.5 prompt; save it in raw and preserve it in the accepted body. Do not place a separate Codex CTA in the frozen source, prompt supplement, or post-expansion body.
...
11. Send the frozen rebuilt manuscript through the standard complete `2.5 Direct Draft` block plus the fixed mother-topic exact-lock block in `3.1-knowledge-base-transplant.md`, using `--isolated`. Do not append `旧2.5风格参照`, a free-form article-specific execution supplement, a safety supplement, route explanation, CTA-free supplement, variable CTA supplement, or another prompt layer. Professional vocabulary and article-specific direction belong in the rebuilt manuscript; the only article-specific prompt slots are the locked mother-topic evidence, source reference count, and selected final count.
...
13. Preserve the raw response and all Gemini-returned content in the accepted body. Do not run conversion cleanup and do not add a Codex soft layer or mid CTA. Record any narrow non-deleting grammar/TTS repairs, then normalize only the terminal ending to the exact current 3.1 fixed ending.
...
- The frozen source uses zero CTA. Gemini may generate the complete old-2.5 conversion units, and every returned unit stays in the accepted body. Codex adds no CTA. Normalize only the terminal ending to `我是探花Gary，我们粉丝群里见，感谢观看。`
...
- Keep terminology density low enough that the body still sounds like natural Gary口播. Do not stack multiple domain labels in one sentence or introduce a new term in every paragraph. Prefer one useful term that explains a scene over three labels that merely rename it.
...
- Apply a hard force-band gate before freezing the rebuilt source. Every replacement point must contain one memorable Gary judgment that a viewer could repeat independently, plus at least one visible interest shift, relationship-position rise/fall, subconscious revaluation, selection result, emotional reward/punishment, or concrete cost. A point that only explains a process, names a skill, recommends communication, manages risk, classifies evidence, or offers balanced advice is incomplete and must be sharpened.
- Across the whole rebuilt source, at least half of the core points must directly expose who gains leverage, who loses bargaining power, what behavior causes a woman to reprice the man, what gets rewarded or punished, or what real consequence follows. The remaining points may teach execution, but they must still serve the source's original attack direction and result promise.
- Reject neutral route shells such as `风险管理`, `关系治理`, `协作系统`, `心理营养`, `能力建设`, `阶段识别`, `证据分类`, `健康沟通`, `边界教育`, `情绪托底`, or equivalent professional-sounding abstractions when they replace the source's sharper promise. These nouns may appear only as subordinate explanations inside a harder argument; they cannot be the central replacement thesis unless the source itself is explicitly about that object.
- Do not confuse harsh vocabulary with a hard viewpoint. `框架`, `主动权`, `拿捏`, `博弈`, `惩罚`, or `暗黑` do not satisfy the force-band gate by themselves. The manuscript must show the behavioral cause, the other person's changed assessment, the resulting position movement, and the practical price or payoff.
- Keep the source's original attack direction and result promise visible in the rebuilt thesis. Do not rebrand a direct topic into neutral process language merely to make the route look new. In particular, do not replace `从认识推进到暧昧/进入男女框架/让她投入/拿回主动权` with organizing labels such as `关系权限逐级开放`, `阶段识别`, `边界管理`, `关系治理`, `健康沟通`, or `风险识别`. New content must remain equally direct, conflict-aware, result-oriented, and provocative while changing its points, mechanisms, operations, scenes, and proof.
...
- rebuilt source or final draft neutralizes the original strong promise by making a safer abstract label the new central thesis, including `关系权限逐级开放`, `阶段识别`, `边界管理`, `关系治理`, `健康沟通`, or an equivalent educational reframe that displaces the source's original attack direction;
- rebuilt source lacks `force_band: old_2.5_hard`, any core point lacks a memorable Gary judgment plus a concrete interest/position/revaluation/consequence movement, or fewer than half of the core points directly expose leverage, bargaining power, repricing, reward/punishment, or real cost;
...
- Gemini raw is overwritten, any returned content unit is deleted or reordered, or an unrecorded wording change appears;
- Codex adds a new CTA, removes or rebuilds Gemini's CTA/account/group language, or summarizes repetitive/promotional material;
- the article withholds its central general answer, uses unsupported fabricated claims, or exposes prompt/backend production language;
- the final body does not end exactly with `我是探花Gary，我们粉丝群里见，感谢观看。`;
- `[[RISKNOTE]]`, yellow-note language, refusal text, or backend traces appear.

Allowed local processing after 3.1 Gemini generation is limited to recorded, non-deleting mechanical repair for unmistakable grammar, punctuation, broken-word, or TTS defects, plus terminal-ending normalization. Do not remove Doubao/AI-summary prompts, interaction prompts, fan-group/contact actions, account introduction, examples, repeated conversion, strong metaphors, dark conclusions, PUA vocabulary, sensational phrasing, or substantive reasoning.
...
如果【原文】来自换芯/换新稿，关系阶段和双方站位必须贴合实际进程。默认把关系外的起点写成“女人还没有进入关系”或“双方仍在陌生/初识阶段”，重点讲男人如何把互动从陌生推进到出现男女意味、暧昧或标题承诺的阶段。不要把女人写成高位的审判者、评委、打分者或验收者，也不要把男人写成上台交卷、等她批准或求她认可的低位角色；不要使用“她在审判你/评判你”作为默认因果框架。保留锋利度，但把冲突落在关系尚未发生、互动还没有推进等真实状态上。若选题确实涉及筛选或判断，只写具体行为，不固化成一方高位裁决、另一方低位受审。
二、成稿目标
...
“好，我们直接进入正题。”之后，正文首段只能承担一个主要任务：直接抛出一个反常识判断、制造一个即时矛盾、揭开一个机制悬念、诊断一个具体行为，或者从结果倒推原因，五者选一，不要在同一段全部完成。共情或性别批判不再作为默认入口，只有原文或本篇结构锁明确指定且没有与近期稿件撞结构时才可以使用。
如果采用痛点式首段，默认控制在 80-140 个中文字符，只保留一个核心痛点、一个狠判断，最多两个能让观众对号入座的行为细节。不要在首段同时解释痛点成因、定义全文机制、预告解决方案、枚举大量场景或重复开头已经承诺的收益。首段的作用是把人钉住，然后立刻推进案例或正文，不是提前讲完整篇文章。
这一规则只限制正文首段的臃肿，不要求每篇都用痛点开场；选择冲突、悬念、诊断、倒推或结论开场时，仍按本篇最合适的动态结构自然推进。完整人物案例默认不得紧跟在开头后面：必须先建立本篇机制命题，并至少展开两个有实质内容的判断、信号或操作区别，再进入有姓名、有情节的案例。只有结构锁明确指定为长案例剖开时才允许案例提前。
...
全文默认使用一到两个小案例。
中段 CTA 前必须完整写出一个贴合本篇文案的学员或兄弟案例，案例结束后再自然进入中段粉丝群引导，不能先写 CTA 再补案例。
这个案例必须完整包含以下六项，缺一项都不算完成：
男性姓名；
女性姓名；
学员原始问题；
我的具体指导、陪跑或咨询；
学员执行后的行为变化；
女生可观察反馈与最终成功结果。
案例要像现实中会发生的事，六项必须通过完整情节自然呈现。不能只写“有个兄弟来找我”，不能省略我的具体介入，也不能用一句笼统的“后来关系变好了”代替女生反馈和最终结果。
以上六项只属于后台完整性合同，不是允许直接照念的公开文案标签。成稿禁止出现“学员原始问题”“他的原始问题”“我的具体指导”“我具体的咨询指导”“我给他的具体咨询指导”等生硬字段词。自然写成“他当时卡在……”“他来找我的时候……”“我让他……”“我当时帮他调整了……”并通过连续情节体现六项信息。
...
案例仍须在中段粉丝群 CTA 之前完整结束，但它可以承担不同功能：贯穿全文、形成失败对照、证明反转或揭开最终机制。普通 2.8 默认不让完整人物案例直接引出正文；先建立机制和至少两个本篇独有判断，再用案例承接和验证。只有结构锁明确指定为长案例剖开时，案例才可以提前。无论承担哪种功能，六项案例信息仍然缺一不可。

核心要求：稳定的是母题、力度、客群、案例合同和结果承诺；变化的是文章如何带着观众走到结论。

正文首段也必须服从结构去同质化：可以从狠判断、反常识结论、冲突现场、机制悬念、行为诊断或结果倒推切入，但只选一个主入口。尤其是痛点式首段，不要连续堆“条件不够、聊天失误、需求感、价值判断、女人反馈、解决方向”等多层解释；把后续层次留给正文展开。不得用完整人物案例替代正文起势。

当本篇结构计划明确选择“共情式”或“批判式”正文首段时，可以写成一个完整大段，不受普通痛点式首段 80-140 个中文字符的默认范围限制。但这种入口只是低频备选，不得在相邻稿件或同一批次里反复出现。如果原文和结构锁没有明确要求共情，默认改用狠判断、机制悬念、行为诊断、即时冲突或结果倒推。这个大段不能只做现象说明，也不能一边批判一边立刻替对方找理由。它必须使用本篇独有的矛盾和因果，禁止继续套用“她拿走了什么、男人承担了什么、进退都被判错”的固定三步骨架。

共情批判的力度来自责任转嫁、标准临时修改和两头有罪的具体矛盾，不来自泛骂“现在的女人”、堆攻击性形容词或扩写一套与标题无关的男女环境。不要用“她只是害怕主动”“她只是缺乏安全感”这类中性解释过早替对方开脱；也不要在段尾马上反过来教育男人，把共情冲淡成“有人不敢判断、有人疯狂脑补”的两边各打五十大板。可以写男人的误判，但应放到后文机制分析里。

如果结构锁确实批准共情入口，写作前只需在内部确认本篇独有的利益矛盾、责任转移和机制回扣，不得把这些检查问题逐项翻译成正文。不同选题必须更换推理动作、场景、利益矛盾和回扣方式，禁止把同一段性别控诉换几个名词后重复使用。
...
六项属于验收信息，不属于公开口播标签。正文不得直接出现“学员原始问题”“他的原始问题”“我的具体指导”“我具体的咨询指导”“我给他的具体咨询指导”等字段化表达，必须改写成自然的故事叙述；如果生成稿仍出现，只在验收环节局部口语化替换，不因此打回重写，也不得改变案例事实、因果链和结果。
...
For automated male relationship expansion, write the selected full instruction block + any required direction-specific injection + source copy to a UTF-8 prompt file. For 3.1, build it with `scripts/build_31_gemini_prompt.py`: the required direction-specific injection is the fixed mother-topic exact-lock block from `3.1-knowledge-base-transplant.md`, placed after the complete 2.5 block and before the source wrapper. For 2.5, 3.1, and the revised 3.2 use `--isolated` per source. Formal 3.5 uses only its staged dedicated Gary candidate session according to `3.5-gary-voice-session.md`. For 2.8, 2.9, and 3.6 use their dedicated-session rules below. Do not run different directions through the same saved conversation.
...
For `3.6 Safe Transplant`, use `--session 3.6`. The prompt must come from `scripts/build_36_gemini_prompt.py`; it contains the complete 2.8 block but expands a 3.5-style frozen replacement route. Keep this session separate from ordinary 2.8 and from the staged 3.5 Gary voice candidate. Use `--isolated` only when the user explicitly requests a clean comparison.

For the revised `3.2 Controlled-Inheritance Theory-First Student-Case Monetization`, use the same expansion path as 3.1: `--isolated`. The original-content scope control, theory-first student cases, and single-CTA monetization logic must already be embodied in the frozen rebuilt source. The Gemini prompt is only the complete current 2.5 direct-draft block plus that frozen source. Do not use `--session 3.2-transplant`, `--session 2.5-transplant`, the default mixed session, or any second-layer 3.2/conversion supplement.
...
- For 3.1, apply its current length threshold to the retained Gemini body. All old-2.5 conversion units remain; Codex adds no CTA and normalizes only the terminal fixed ending.
...
For a short, mother-topic-drifted, or count-drifted 3.1 output, rebuild the complete prompt with the same fixed mother-topic exact-lock block and rerun it through `--isolated`. CTA, Doubao, group, account, or engagement content is retained and is not a retry reason. Use this retry instruction before the source wrapper:

```text
上一稿未通过3.1扩写验收。请完整重写到当前字数要求。公开母题必须逐字使用提示词锁定的【公开母题】，第一句和收束必须出现该原句，强制片头中的【文案核心】也必须直接使用该原句。正文内部机制只能放在母题下面解释，不得替换、概括或先于公开母题组织全文；以提示词中的【本轮成稿数量承诺】为唯一数量合同，必须完整兑现且不得临时增减，原稿数量仅供参考。
```
...
- Output ending violates the selected direction. Ordinary Gary directions follow their own fixed ending. For 3.1, preserve all Gemini CTA/account/group content and normalize only the terminal ending to `我是探花Gary，我们粉丝群里见，感谢观看。`
