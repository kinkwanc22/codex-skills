...
- `3.5 Codex-First, Knowledge-Supported Hybrid -> Mother-Topic-Locked 2.5 Expansion` / `3.5混合换芯`: use only when the user explicitly asks for 3.5, Codex先设计＋知识库辅助, or the approved September 3 hybrid logic. Read `references/3.5-codex-first-knowledge-supported.md` and `references/3.5-gary-voice-session.md` in full. Codex designs one coherent article-level spine; knowledge supports mechanisms, proof, language, and one or two sourced Gary student mini-cases. Build with `scripts/build_35_gemini_prompt.py`, stage the dedicated Gary voice session into the work package, run the candidate with `--session-file`, and narrow unsupported model-added case facts back to the frozen evidence boundary. Do not use `--isolated`, `--new-session`, or the mixed default session for formal 3.5.
