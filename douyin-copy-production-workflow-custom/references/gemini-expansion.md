...
- `3.1 Knowledge-Base Transplant -> Mother-Topic-Locked 2.5 Expansion + Conversion Cleanup` / `2.5原稿预换芯`: the canonical default for 3.1. Read `references/3.1-knowledge-base-transplant.md` in full. Codex builds the frozen source only from traceable knowledge. Gemini remains unrestricted by that knowledge closure and receives the complete standard 2.5 prompt, the fixed mother-topic exact-lock block, and the frozen source through `--isolated`. Save the raw response, remove old-2.5 conversion units into a separate cleaned body with an exact manifest, then let Codex add exactly one mid CTA and the fixed ending. Do not add scattered soft placements.
...
紧接着必须植入引导语：“另外说一下，想节省时间的话，可以点个收藏，在评论区艾特豆包，让豆包给你总结出精髓或者思维导图后再回来观看，如果现实中有任何推进问题的话，也可以随时进入我的粉丝群提问。”
...
9. Keep conversion logic out of the rebuilt manuscript. Gemini may generate only the conversion language already required by the complete old 2.5 prompt; save it in raw and remove it before content acceptance. Record the topic-specific evidence package and bounded group-delivered decision in the run metadata so Codex can write the new mid CTA after expansion; do not place that CTA in the frozen source or prompt supplement.
...
13. Preserve the raw response, delete clearly separable old-2.5 conversion units into a new artifact with an exact manifest, and recheck the cleaned zero-CTA body against the topic fingerprint and recent cross-article routes. Retry only if cleanup cannot preserve substantive content or another content gate fails. After cleaned-body acceptance, Codex uses the recorded conversion signature to write the soft layer and mid CTA.
...
- The frozen source and cleaned Gemini body use zero CTA. Preserve Gemini raw even when it contains the complete old-2.5 conversion units, remove those units exactly with a manifest, then let Codex write one natural mid-article diagnostic CTA and append only the exact fixed ending; do not add a second dynamic ending CTA by default.
...
- Gemini raw is overwritten, exact conversion deletions are not manifested, or conversion cannot be isolated without deleting substantive content;
- the cleaned Gemini body contains any CTA, `粉丝群` invitation, `进群/关注/主页/私信/咨询` action, account self-introduction, or fixed ending;
...
- Codex does not append the exact fixed ending after the cleaned zero-CTA body is accepted;
- `内部群`, `私董会`, `[[RISKNOTE]]`, yellow-note language, refusal text, or backend traces appear.

Allowed local processing after 3.1 Gemini generation has two distinct parts. First, remove clearly separable conversion units verbatim into a deletion manifest; this includes Doubao/AI-summary prompts, interaction prompts, fan-group/contact actions, account introduction, and the old ending. Second, allow only narrow mechanical repair for unmistakable transcription/typing errors, prompt/backstage leakage, or a minimal edit to remove explicit coercion, unlawful conduct, ignoring refusal, or physical-boundary violations. Neither part permits rewriting tone, metaphors, intensity, dark conclusions, PUA vocabulary, sensational phrasing, or substantive reasoning. If conversion is fused into a substantive sentence or a hard operational boundary controls a whole section, rerun from the rebuilt source instead of damaging or neutralizing the article.
...
- For 3.1, apply its current length threshold to the cleaned Gemini body. The raw output may contain old-2.5 conversion units; the cleaned body must contain zero CTA and zero fixed ending. Codex adds the final conversion and ending after acceptance.
...
For a short, mother-topic-drifted, count-drifted, or non-isolatable-conversion 3.1 output, rebuild the complete prompt with the same fixed mother-topic exact-lock block and rerun it through `--isolated`. CTA contamination alone is handled by the deterministic cleanup and is not a retry reason. Use this retry instruction before the source wrapper:

```text
上一稿未通过3.1扩写验收。请完整重写到当前字数要求。公开母题必须逐字使用提示词锁定的【公开母题】，第一句和收束必须出现该原句，强制片头中的【文案核心】也必须直接使用该原句。正文内部机制只能放在母题下面解释，不得替换、概括或先于公开母题组织全文；数量承诺必须完整兑现且不得增减。
```
...
- Output ending violates the selected direction. Ordinary Gary directions must contain their required exact fixed ending and reject `内部群`; for 3.1, old-2.5 ending/CTA is allowed only in the immutable raw artifact, while the cleaned Gemini body must contain neither because Codex appends the exact final conversion layer after acceptance.
