...
Always paste this full instruction block before the original source copy whenever sending a script to the local Gemini command-line chat for expansion, including first attempts, refusal retries, length retries, and hallucination/stale-topic retries. Do not send `/new` unless the user explicitly asks:
...
For automated expansion, write the full instruction block + source copy to a UTF-8 prompt file, then call the same command with `--prompt-file --isolated`:
...
- Paste the full block from `Current Gemini Expansion Instruction` followed by the original source copy into the terminal chat, or use `--prompt-file` for automated runs.
- Do not prepend a viral opening or rewrite the source before expansion; the only required pre-source instruction is the full `Current Gemini Expansion Instruction` block.
...
If Gemini returns a refusal, safety block, tool/API failure text, or a non-draft answer instead of an expanded article, treat it as a failed generation and retry directly in the current terminal session. Send the original source copy again with the full `Current Gemini Expansion Instruction` block. Do not ask the user for confirmation.
...
For automated runs, prefer building a new UTF-8 retry prompt file with the full `Current Gemini Expansion Instruction` block + original source copy + retry reason, then rerun `run_gemini_chat.cmd --prompt-file <retry-prompt> --isolated`. Treat this as the normal automated equivalent of a same-source retry.

If obvious hallucination appears, retry in the same terminal session with the full `Current Gemini Expansion Instruction` block and the original source copy. Record the failure in the batch log.
