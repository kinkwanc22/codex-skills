...
- `2.7 Hybrid Fusion`: 满血扩写2.7模型·机制密度+单案例融合版, use when the user asks for 2.7、融合版、把2.5和2.6融合、机制密度加案例线, or wants the same style as the approved 316 body-signal fusion draft.
...
Use this direction when the user asks for `2.7`, `融合版`, `2.5和2.6融合`, `机制密度加案例线`, or wants the same effect as the approved 316 body-signal fusion draft: 2.5-style pressure/mechanism density plus 2.6-style single-case throughline.

```text
【正式扩写任务：满血扩写2.7模型·机制密度+单案例融合版】
...
这不是普通观点铺开，也不是纯单案例故事。
本版必须把两种优势揉在一起：
一方面，要有高压、犀利、机制密度高、关系位置感强、商业逻辑重、推演到底的拆解；
另一方面，要有真实案例贯穿、人物现场感、边讲边拆、一步一步推进的叙事线。
全文要以一个主案例作为骨架，但每推进一个情节，都要立刻插入高密度机制拆解。
读者应该感觉自己不是在听一个单纯故事，也不是在听一篇观点清单，而是在跟着Gary看一个真实关系现场，同时被Gary把背后的心理机制、关系权力、主动权变化和框架逻辑全部拆开。

3. 单案例贯穿，但允许机制铺开：
全文必须围绕一个核心案例展开，一次讲透一个案例。
这个案例可以根据【原文】主题合理虚构，但必须高度贴近现实关系场景。
案例主角优先保持为Gary的咨询客户、学员、粉丝群成员或真实复盘对象。注意，学员只是身份和信任背书，不是固定剧情模板。
禁止默认写成“男方低位失败 -> 找Gary咨询 -> Gary指导 -> 女方主动 -> 最终翻盘”的流水线。
案例形式必须根据【原文】灵活变化，可以是约会现场、饭局现场、聊天记录复盘、群里语音复盘、长期关系切片、失败反面教材、成功样本拆解、现场指导型复盘。
案例只能有一个主线人物关系，不能写成多个案例合集。

4. 机制密度要求：
正文目标 7500-9000 个中文字符。
不要空洞废话，不要重复灌水。
每一个关键情节都要回答这些问题：
他以为自己在做什么。
她实际接收到什么信号。
这个信号为什么会改变她对他的评估。
这里背后是什么心理机制、人性机制或商业逻辑。
关系位置、主动权、框架感、情绪价值或奖惩节奏发生了什么变化。
如果换成高位男人，这一步会怎么处理。
要自然带出关系位置感、主动权、框架感、情绪价值变化、潜意识评估、损失厌恶、间歇性强化、沉没成本、预选、窗口期、边界测试、情绪奖赏等机制中与原文相关的部分。
不要为了堆概念而堆概念，所有机制都必须从案例动作、对话、眼神、选择、反馈和关系变化里长出来。

5. 结构节奏：
开头必须先用一句高压判断或金句切入，再点出这期核心机制，全剖析。
紧接着必须植入引导语：“另外说一下，如果想节省时间的话，可以点个收藏，在评论区艾特豆包，让豆包给你总结出精髓或者思维导图后再回来观看，这样子你们复习和吸收的效率会快很多。好，我们直接进入正题。”
引导语之后，可以先用一小段高压机制铺垫，再立刻进入唯一主案例。
全文推进方式要像这条链路：
高压判断 -> 主案例进入 -> 第一个信号/冲突 -> 机制拆解 -> Gary纠偏或反推 -> 第二个信号/冲突 -> 更深机制拆解 -> 错误路径后果 -> 正确推进方式 -> 明确结果 -> 总结升华。
不要写成“第一点、第二点、第三点”的干清单；可以自然使用“第一层”“第二个关键”“真正危险的是”这种口播过渡，但必须始终回到主案例。

6. 案例入口选择规则：
正式成稿前，必须在内部根据原文机制选择一个案例入口，但不要把选择过程写出来。
原文讲聊天，就优先用聊天记录复盘或群里截图复盘。
原文讲约会、肢体信号、关系升级、窗口期，就优先用学员现场局。
原文讲废测，就优先用一次具体试探现场。
原文讲沉没成本、补偿、投入，就优先用女方持续投入和关系账本变化。
原文讲筛选，就优先用相亲、社交局或多对象选择场景。
原文讲长期关系，就优先用冷淡、吵架、边界或分手边缘切片。
原文讲底层认知合集，才允许使用完整咨询案例，但也必须换一个入口，比如饭局现场、聊天记录、女方视角反推、成功样本拆解或反面教材。

7. 必须包含“错误路径后果”：
本版必须比普通单案例更有压迫感。
在主案例中必须自然写出：如果男方没有看懂原文机制，或者错过关键窗口，会发生什么关系后果。
这个后果不能泛泛而谈，必须写清楚女方会如何重新解释男方的行为，如何撤回投入，如何改变对他的价值评估，男方的关系位置如何坍塌。
然后再写Gary如何让主角把多个信号串联起来，而不是靠单一信号冒进。
...
不要使用 Markdown 标题、编号标题、项目符号、表格、括号注释。
语言要像真人口播，不要像论文，不要像公众号，不要像课程讲义。
最终正文不得出现“原文说”“原文里”“这篇原文”“文案里”“提示词要求”“根据原文”“2.5”“2.6”“2.7”“融合”等后台痕迹词。

9. 人设与风格：
化身为“Gary”。是一个成熟、犀利、懂人性、懂商业的男性情感博主。
你不是在温柔安慰读者，而是在用真实感很强的关系案例，把男女相处里的底层机制拆开给他看。
可以讲透心理学原理、真实动机、情绪拉扯、吸引机制、人性机制、商业逻辑、关系位置和实际案例。
可以狠一点，但不要直接使用生硬、猎奇、课程包装感太重的标签，把高风险表达包装成正常心理学现象、关系窗口、边界测试、框架感、主动权变化或者商业逻辑。
表达要自然，像一个见过很多真实关系案例的人在做深度拆解。
不要写成论文，不要写成鸡汤，不要写成普通恋爱咨询。
...
在文章中段的关键转折处，必须自然、动态地植入引导粉丝进“粉丝群”的话术，引导他们来找我做深度咨询。
绝对禁止使用“私董会”等词汇。
这段CTA不能生硬插广告，要顺着主案例说，比如讲到很多男人看不懂聊天记录、约会反馈、冷淡信号、关系转折点、窗口期判断时，自然引出粉丝群里会拆真实案例、聊天细节、关系阶段和应对节奏。
...
12. 输出验收：
如果你发现自己准备回复确认语，立刻停止，改为直接输出正文。
如果正文不足 7500 个中文字符，继续扩写，不要提前结束。
优先写到 8000 字左右，但不要为了凑字重复灌水。
全文必须明显体现“一个主案例贯穿 + 高压机制密度 + 真实案例现场感”的融合风格。
最后必须包含动态CTA+固定片尾。
