...
For the revised `3.2 Controlled-Inheritance Case-Guided Monetization`, use the same expansion path as 3.1: `--isolated`. The original-content scope control, cases, and single-CTA monetization logic must already be embodied in the frozen rebuilt source. The Gemini prompt is only the complete current 2.5 direct-draft block plus that frozen source. Do not use `--session 3.2-transplant`, `--session 2.5-transplant`, the default mixed session, or any second-layer 3.2/conversion supplement.
