...
- `3.1H Codex-First, Knowledge-Supported Hybrid -> Mother-Topic-Locked 2.5 Expansion` / `3.1H混合换芯`: use only when the user explicitly asks for 3.1H, Codex先设计＋知识库辅助, or the approved September 3 hybrid logic. Read `references/3.1H-codex-first-knowledge-supported.md` in full. Codex designs one coherent article-level spine; knowledge supports mechanisms, proof, language, and one or two sourced Gary student mini-cases. Build with `scripts/build_31h_gemini_prompt.py`, run `--isolated`, and narrow unsupported model-added case facts back to the frozen evidence boundary.
