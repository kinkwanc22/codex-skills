...
- `3.2 Case-Guided Source Pre-Transplant -> 2.5 Direct Draft` / `3.2换芯扩写`: use when the user asks for 3.2、3.2换芯、3.2换芯扩写、带小案例的预换芯, or wants the complete 3.1 architecture with short cases built into the replacement source. Read and follow `references/3.2-conversion-transplant.md` in full. Freeze the rebuilt source first, then send only the complete 2.5 block plus that source through `--isolated`; never append a 3.2 task layer to Gemini.
...
For the revised `3.2 Case-Guided Source Pre-Transplant`, use the same expansion path as 3.1: `--isolated`. The complete 3.2 logic must already be embodied in the frozen rebuilt source. The Gemini prompt is only the complete current 2.5 direct-draft block plus that frozen source. Do not use `--session 3.2-transplant`, `--session 2.5-transplant`, the default mixed session, or any second-layer 3.2 supplement.
