...
- `3.2 Controlled-Inheritance Theory-First Student-Case Monetization -> 2.5 Direct Draft` / `3.2换芯扩写`: use when the user asks for 3.2、3.2换芯、3.2换芯扩写、带小案例的预换芯, or wants 3.1 with controlled original-content inheritance, compact Gary student-case proof after selected theory explanations, and the approved single-CTA monetization sequence. Preserve 60%-80% of the source's content functions and overlay conversion on its original answer path; do not write a different article under the same title. Read and follow `references/3.2-conversion-transplant.md` in full. Freeze the rebuilt source first, then send only the complete 2.5 block plus that source through `--isolated`; never append a 3.2 task or conversion layer to Gemini.
