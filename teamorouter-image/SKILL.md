...
description: Generate or edit images through the TeamoRouter GPT Image 2.5 Sunburst API. Use when a user or agent asks to call TeamoRouter, Sunburst, gpt-image-2.5-sunburst, or this local image API wrapper.
---

# TeamoRouter Image

Use the `scripts/teamorouter_image.py` file resolved relative to this `SKILL.md` as the deterministic API client; do not assume the caller's working directory is the skill folder. It supports generation, editing, local size validation, dry runs, and secure local credential configuration without third-party Python packages.
...
3. For edits, supply an existing image and optionally a same-size mask:
...
- Model is always `gpt-image-2.5-sunburst`.
