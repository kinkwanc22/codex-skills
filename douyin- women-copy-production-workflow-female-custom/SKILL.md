...
If the user asks for `2.5成稿换芯`, `用扩写好的2.5改内容`, `保留母题替换内容`, or says to keep the method approved by the August 1 fixed-mother-topic batch, use the current canonical fixed-mother-topic limited-inheritance mode in `references/2.5-finished-draft-transplant.md`. Prompt assembly must begin with the female override and the complete early-B 2.5 block; then append the transplant task layer, an unused route, a limited-inheritance card, cumulative route exclusions, and the approved 2.5 finished draft. Run every article through `--session-file work/gemini_session_female_25_transplant_legacy.json`; never use the male shortcut or male session file.
