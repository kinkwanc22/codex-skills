...
- If the output reuses a previous article's theme, switches topic, or comments on the source instead of expanding it, treat it as hallucination/running off-topic; retry with the full instruction and original source in a new isolated prompt file.
...
- If Gemini returns below 1000 Chinese characters, do not mark it passed. Retry with the full instruction and original source in a new isolated prompt file, or mark it as not entering the final Word pool if retries trigger refusal.
