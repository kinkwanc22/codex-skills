...
- Treat Lovart's `generate_image_gpt_image_2*` callable tool ids as the GPT Image 2 family.
- Prefer `generate_image_gpt_image_2_medium`, the GPT Image 2 Medium route; do not fall back to Auto, Low, High, GPT Image 2.5, or another model family merely because the user omitted quality.
...
The local runner passes the GPT Image 2 Medium tool id through `--prefer-models` and puts the model family, exact count, and dimensions in the prompt parameter line. Use its `--dry-run` / `--print-prompt` mode for offline prompt and parameter checks; that mode must not upload files or make a network call.
