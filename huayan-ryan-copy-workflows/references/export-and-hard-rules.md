...
- The current default final output folder is the dual-device synced workflow folder: `/Users/kin/工作用（同步）/花研Ryan/成品` on Mac, and `D:\工作用（同步）\花研Ryan\成品` on Windows. The old Windows folder `E:\工作用\素材文稿\codex工作流长文稿` is legacy-only unless the user explicitly asks for it.
...
- Current synced output folder is `/Users/kin/工作用（同步）/花研Ryan/成品` on Mac and `D:\工作用（同步）\花研Ryan\成品` on Windows; save generated `.docx` results there by default so both devices can pick them up.
...
- For first attempts, refusal retries, stale-topic retries, hallucination retries, and length retries, always send the Female Edition Prompt Override + selected full Gemini expansion instruction block + its documented direction-specific injection + the original source copy wrapped as `【原文开始】...【原文结束】`; do not prepend `/new` unless explicitly requested. For 2.5, rerun the complete prompt with `--isolated`.
...
- `2.5 Direct Draft` uses the complete `Female Edition Prompt Override` + full restored early-B 2.5 instruction block + source manuscript, with one `--isolated` request per source. Do not append `旧2.5风格参照` in routine production unless the user explicitly asks for a style-calibration comparison. For `3.1 Source Pre-Transplant`, embed 2-5 total matched terms directly in the rebuilt source, freeze it, then use only the female override + standard complete 2.5 block + frozen rebuilt source. Never append an article-specific supplement or safety layer in 3.1.
...
- For automated expansion, prefer the selected host runner with a UTF-8 prompt file containing the Female Edition Prompt Override + selected full instruction block + its documented direction-specific injection + original source copy wrapped as `【原文开始】...【原文结束】`.
- Do not open or operate the old web expansion channel in this custom skill.
- Before every new queued 2.5 source, paste the complete `Female Edition Prompt Override`, the selected full Gemini expansion instruction block, and then the original source copy ending with `【原文结束】`; run it with `--isolated`. Do not add the short legacy injection unless the user explicitly requests a style-calibration comparison, and do not send `/new` unless explicitly requested.
...
- Do not prepend any extra instructions beyond the stable `Female Edition Prompt Override`, the selected full mandatory Gemini block, and its documented direction-specific injection, unless a retry instruction is explicitly needed.
