...
- The semi-transparent title in the upper-left is opening-only. It must begin at the start of the timeline and extend through the complete opening, ending exactly at the same semantic body boundary. Verify `opening_title_end == dynamic_video_end == body_start == still_image_start == body_packaging_start == yellow_icon_start`.
