---
name: jiepai-jianji
description: Local street-shot and 自然流千川 editing workflow for Chinese narration videos and Jianying drafts. Use when the user asks for 街拍剪辑, 自然流千川, 街拍素材随机剪辑, 口播音频配素材, 按句尾切换素材, 静音点切换素材, 逐字对齐剪视频, 剪映街拍草稿, or wants to turn a video-material folder plus narration audio plus script/docx into a 16:9 horizontal 30fps video.
---
...
- For `自然流千川`, female-lead selection is globally consumptive across delivered drafts and batches. Before planning a batch, read `/Users/kin/工作用（同步）/图/长视频用图/2026-08-27_56位女主_Lovart五套竖屏/.records/natural_flow_used_female_leads.json` and exclude every recorded lead ID. Compare by the stable three-digit lead ID, not the descriptive folder suffix, because `(带视频)` and renamed aliases can refer to the same woman. Assign a different unused lead to every draft in the new batch; once a draft has been created and exposed to the user for review, append that lead to the ledger even if the draft is later deleted or replaced. Never treat deletion, renaming, a different clip order, or a different folder alias as making the lead unused again.
- If the remaining unused leads do not provide enough horizontal, visually safe video-and-still sets for the whole batch, stop before writing or replacing drafts and report the exact shortage. Do not silently recycle a used lead, repeat one new lead across several drafts, substitute revealing/unsafe footage, crop vertical footage into the horizontal lane, or synthesize new paid footage without explicit user direction.
